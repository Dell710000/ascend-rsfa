"""
Ablation Study Version 01: Official (Baseline)
================================================
Vanilla FlashAttention V2 implementation - TritonAscend official baseline.
Optimizations: NONE
- K block: [BLOCK_N, HEAD_DIM] with explicit tl.trans
- K/V pointers advanced at END of each loop iteration
- Fixed 20 AI cores, persistent scheduling with stride 20
- No compile pipeline configuration
...
# ---------- Inner loop (Official: K transpose, K/V advance at end) ----------
@triton.jit
def _attn_fwd_inner(
    acc_ptr, l_i, m_i, q,
    ...
    # Official: K is [N_CTX, HEAD_DIM], advance along first dim
    K_block_ptr = tl.advance(K_block_ptr, (lo, 0))
    V_block_ptr = tl.advance(V_block_ptr, (lo, 0))

    for start_n in range(lo, hi, BLOCK_N):
        start_n = tl.multiple_of(start_n, BLOCK_N)
        # -- Load K, then transpose --
        k = tl.load(K_block_ptr)        # [BLOCK_N, HEAD_DIM]
        trans_k = tl.trans(k)           # [HEAD_DIM, BLOCK_N] - explicit transpose
        qk = tl.dot(q, trans_k)
        ...
        # -- Advance both pointers at END of iteration --
...
# ---------- Main kernel (Official: fixed 20 cores, K as [N_CTX, HEAD_DIM]) ----------
@triton.jit
def _attn_fwd(
    ...
    # Official: fixed 20 cores persistent scheduling
    for block_idx in range(pid, NUM_BLOCKS, 20):
        ...
        # Official: K as [N_CTX, HEAD_DIM] - same logical layout as storage
        ...
        # STAGE=1: non-causal (inner STAGE=3 -> full range)
        # STAGE=3: causal = stage1(prefix) + stage2(diagonal)
        if STAGE & 1:
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale, BM, BN):
        ...
        # Official: fixed 20 cores, no compile pipeline config
        _attn_fwd[(20,)](
            q, k, v, M, o, sm_scale,
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, dtype, BM, BN):
    ...
    print(f"[PASSED] Official: Z={Z} H={H} N={N_CTX} d={HEAD_DIM} causal={causal}")


if __name__ == "__main__":
    print("=== Ablation 01: Official (Baseline) ===")
    # Causal tests
