"""
Ablation Study Version 02: V1 (+ K/V Address Advance)
======================================================
Official baseline + K/V address advance (Load-Advance software pipeline).
Effective optimization: K/V address advance
- K pointer advanced IMMEDIATELY after load (enables earlier address resolution)
- V loading adaptive:
  * HEAD_DIM >= 128, non-diagonal: load V early (after QK, before softmax)
  * HEAD_DIM < 128 or diagonal: load V late (after softmax)
- Fixed 20 AI cores
...
# ---------- Inner loop (V1: K advance immediately after load, V adaptive) ----------
@triton.jit
def _attn_fwd_inner(
    acc_ptr, l_i, m_i, q,
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        # -- V1: Load K, advance K IMMEDIATELY for next iteration --
        k = tl.load(K_block_ptr)
        K_block_ptr = tl.advance(K_block_ptr, (0, BLOCK_N))
        qk = tl.dot(q, k)

        # -- V1: Adaptive V loading - early for large d (overlap with softmax) --
        ...
        # -- V1: Adaptive V loading - late for small d or diagonal --
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, dtype, BM, BN):
    ...
    print(f"[PASSED] V1: Z={Z} H={H} N={N_CTX} d={HEAD_DIM} causal={causal}")


if __name__ == "__main__":
    print("=== Ablation 02: V1 (+ K/V Address Advance) ===")
