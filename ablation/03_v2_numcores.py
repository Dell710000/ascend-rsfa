"""
Ablation Study Version 03: V2 (+ numcores Dynamic Selection + Persistent Scheduling)
=====================================================================================
V1 + Dynamic AI core selection + Persistent scheduling.
Optimizations: K/V address advance + numcores dynamic + persistent
- K pointer advanced IMMEDIATELY after load
- V loading adaptive (early for large d, late for small d/diagonal)
- Dynamic num_cores: min(available AI cores, total tasks)
- Persistent scheduling with dynamic stride (num_cores instead of fixed 20)
...
# ---------- Inner loop (V2: same as V1 - K advance immediately, V adaptive) ----------
@triton.jit
def _attn_fwd_inner(
    acc_ptr, l_i, m_i, q,
...
# ---------- Main kernel (V2: dynamic num_cores persistent scheduling) ----------
@triton.jit
def _attn_fwd(
    ...
    pid = tl.program_id(0)
    # V2: dynamic num_cores from grid, persistent scheduling with dynamic stride
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale, BM, BN):
        ...
        # V2: dynamic grid based on available AI cores and total tasks
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, dtype, BM, BN):
    ...
    print(f"[PASSED] V2: Z={Z} H={H} N={N_CTX} d={HEAD_DIM} causal={causal}")


if __name__ == "__main__":
    print("=== Ablation 03: V2 (+ numcores Dynamic + Persistent) ===")
